...
{
	//printf("rili");
	int total_processos, id, ndades = atoi(args[1]);
	...
	int x=2;
	while(id%x==0 && x<total_processos)
	{
		x=x*2;
	}
	int num_received_values, i, counter = 0, proces_objectiu, porcio = ndades / total_processos, residu = ndades % total_processos, acumulador = 0;
	int *vin, *vout, *vtmp;
		...
		else
		{
			proces_objectiu=id+i/2;			
				//printf("Soy %d, quiero recibir %d de %d, espero \n", id, i/2*porcio, proces_objectiu); 
			        //fflush(stdout);
			MPI_Recv(&vin[i/2*porcio], i/2*porcio, MPI_INT, proces_objectiu, 0, MPI_COMM_WORLD, &estat);
