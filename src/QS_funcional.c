...
int main(int nargs,char* args[])
{
	...
	//fflush(stdout);
	int * valors=malloc((porcio+1)*sizeof(int));  
	int * valors2=malloc((porcio+1)*sizeof(int));  
	//printf("Caca %d \n", id);	
	//fflush(stdout);
        ...
        for (i=2; i<total_processos*2; i*=2)
	{
		...
		//	        fflush(stdout);
				MPI_Recv(vin2, ndades/2, MPI_INT, proces_objectiu, 0, MPI_COMM_WORLD, &estat);
		//		printf("Soy %d, he recibido %d de %d, espero \n", id, i/2*porcio, proces_objectiu); 
		//	        fflush(stdout);
				int elementos = merge2_different(vin, i/2*porcio, vin2, i/2*porcio, vout);
		//		printf("Soy %d, he mergeado %d con %d \n", id, elementos, proces_objectiu); 
		//	        fflush(stdout);
				valors = realloc(valors,elementos*sizeof(int));
				valors2 = realloc(valors2,elementos*sizeof(int));
				memcpy(valors, salida, sizeof(salida));
				//memcpy(valors2, salida, sizeof(salida));
				vin = valors;
				vin2=valors2;
				salida = realloc(salida,((porcio*i*2)+1)*sizeof(int));
				vout=salida;
